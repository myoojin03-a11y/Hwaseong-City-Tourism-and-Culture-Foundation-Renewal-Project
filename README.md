...
### 🗺️ 하이브리드 관광지 슬라이더 (Hybrid Tour Guide Slider)
<img src="https://github.com/user-attachments/assets/a5b2fa33-19cb-48e9-9a62-8432b23b823d"/>
<img src="https://github.com/user-attachments/assets/a5b2fa33-19cb-48e9-9a62-8432b23b823d"/>

### 📌 기능 개요
PC와 모바일 환경에 최적화된 인터페이스를 각각 별도로 구현하여, 화성의 주요 관광지를 효과적으로 소개하는 슬라이더 세션입니다.

<br>

#### 1. CSS & UI/UX 디자인
*   **사용자 참여 유도 (CTA 애니메이션):** `keyframes`를 활용한 무한 반복 애니메이션(`click_anim`)을 탭 메뉴에 적용하여 사용자의 클릭 인터랙션을 자연스럽게 유도했습니다.
*   **유연한 모바일 레이아웃:** `flex-shrink: 0`과 `overflow-x: auto`를 조합하여 모바일 기기에서 가로 스크롤이 가능한 탭 네비게이션을 구현했습니다.
*   **시각적 피드백:** 현재 활성화된 슬라이드 상태를 직관적으로 알 수 있도록 **상단 프로그레스 바**와 **플로팅 라벨(`current-label`)**을 결합하여 가독성을 높였습니다.
*   **반응형 스타일링:** `display: none`과 `display: block`을 활용하여 기기 환경(PC/Mobile)에 따라 슬라이더 구조가 완전히 교체되는 적응형 UI를 설계했습니다.

#### 2. JavaScript (PC - Swiper.js)
*   **복합 상태 동기화:** Swiper의 `realIndex`를 추적하여 메인 슬라이드와 썸네일 버튼의 `active` 상태를 실시간으로 일치시켰습니다.
*   **커스텀 인터페이스:** `renderFraction` 메서드를 통해 기본 페이지네이션을 커스텀하여 브랜드 아이덴티티에 맞는 숫지 표기법을 적용했습니다.

#### 3. JavaScript (Mobile - Vanilla JS)
*   **자동재생 엔진:** `setInterval`을 활용한 커스텀 자동재생 기능을 구현하고, 재생/일시정지 버튼(`toggleAutoplay`)을 통해 사용자가 제어권을 가질 수 있도록 설계했습니다.
*   **스크롤 포커싱:** 탭 클릭 시 `scrollIntoView({ behavior: 'smooth' })`를 적용하여 선택된 카테고리가 항상 화면 중앙 혹은 가시 영역에 위치하도록 처리했습니다.
*   **스크롤 락(Scroll Lock):** 팝업 및 모달 활성화 시 `body`의 `overflow: hidden` 처리를 통해 불필요한 배경 스크롤을 차단했습니다.
